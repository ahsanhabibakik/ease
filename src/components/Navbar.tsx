      ...
      <div className="backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/90 border-b border-gray-200/70">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-6">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {status === 'loading' && (
              <div className="text-xs text-gray-500 px-2">...</div>
            )}
            {status !== 'loading' && !session && (
              <button onClick={() => signIn()} className="inline-flex items-center gap-2 rounded-lg border border-gray-300/60 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 ring-accentTeal/40">Sign In</button>
            )}
            {session && (
              <div className="flex items-center gap-2">
                {session.user?.image && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={session.user.image} alt={session.user.name || 'User'} className="h-9 w-9 rounded-full ring-2 ring-accentTeal/30 object-cover" />
                )}
                <button onClick={() => signOut()} className="inline-flex items-center gap-2 rounded-lg border border-red-300/60 bg-white px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 focus:outline-none focus-visible:ring-2 ring-red-300/60">Sign Out</button>
              </div>
            )}
          </div>
      ...
      {mobileOpen && (
        <div className="md:hidden border-b border-gray-200/70 dark:border-gray-700 bg-white/95 dark:bg-gray-900/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:supports-[backdrop-filter]:bg-gray-900/80 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col gap-2">
            ...
            <button onClick={toggleTheme} className="mt-2 inline-flex items-center justify-center rounded-lg border border-gray-300/60 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 px-4 py-2 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus-visible:ring-2 ring-accentTeal/40">Toggle Theme</button>
            ...
            {status !== 'loading' && !session && (
              <button onClick={() => signIn()} className="mt-2 inline-flex items-center justify-center rounded-lg border border-gray-300/60 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 ring-accentTeal/40">Sign In</button>
            )}
            {session && (
              <button onClick={() => signOut()} className="mt-2 inline-flex items-center justify-center rounded-lg border border-red-300/60 bg-white px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 focus:outline-none focus-visible:ring-2 ring-red-300/60">Sign Out</button>
            )}
          </div>
        </div> )}
    </header>
