...
export default function SignInMenu({ onClose, className }: SignInMenuProps) {
  ...
  return (
    <div ref={ref} className={cn('w-72 rounded-xl border border-[var(--c-border)] bg-[var(--c-surface)] shadow-lg p-3 flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 origin-top', className)}>
      <p className="text-xs font-medium tracking-wide text-[var(--c-text-faint)] px-1">Sign in with</p>
      ...
      {emailProvider && (
        <form onSubmit={handleEmail} className="pt-1 space-y-2">
          <label className="text-[10px] font-medium uppercase tracking-wide text-[var(--c-text-faint)] px-1">Email</label>
          ...
          <Button type="submit" size="sm" variant="primary" loading={submitting} className="w-full">Send Magic Link</Button>
        </form>
      )}
